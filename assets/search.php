...
<?php $c_sql = mysql_query("SELECT * FROM click_comerciales"); while($c = mysql_fetch_assoc($c_sql)){ ?>
    window.index.add({
      href: '<?php echo $config['site']; ?>/<a style="color: <?php echo $config['colorsv']; ?>;" href="index.php?page=gerentes&edit_comercial=<?php echo $c['id']; ?>"><?php echo $c['nombre']; ?> <?php echo $c['primer_apellido']; ?> (<?php echo $c['subgerente']; ?>)',
	  span: 'nepe',
