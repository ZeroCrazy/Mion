...
$estilo = array( 
 ...
 $objXLS->getActiveSheet()->getStyle('A1:BP1')->applyFromArray($estilo);
// Color de fondo 'Casilla', 'Color'
// ###### COLOR AZUL ######
cellColor('A1:AL1', '0404fc');
cellColor('AT1', '0404fc');
cellColor('BM1', '0404fc');
// ###### COLOR VERDE ######
cellColor('AM1:AS1', '008000');
cellColor('AU1:BL1', '008000');
cellColor('BN1:BP1', '008000');
...
$objSheet->setCellValue('BN1', 'CAIDOS');
$objSheet->setCellValue('BO1', 'MOTIVO');
$objSheet->setCellValue('BP1', 'WE');
	...
	if($_GET['gerente']){
		...
		$can=mysql_query("SELECT * FROM contratos_voids WHERE we_dia='$_GET[we_dia]' AND we_mes='$_GET[we_mes]' AND we_ano='$_GET[we_ano]'");
	}
	...
	while($dato=mysql_fetch_array($can)){
		...
		$objSheet->setCellValue('W'.$numero, '');
		...
		$objSheet->setCellValue('BN'.$numero, 'VOID');
		$objSheet->setCellValue('BO'.$numero, ' ' . $dato['motivo']);
		$objSheet->setCellValue('BP'.$numero, $dato['we_dia'] . '/' . $dato['we_mes'] . '/' . $dato['we_ano']);
	}
