...
$objSheet->setCellValue('CQ1', 'EDP_CLICK');
	...
	$numero=1;
	$can=mysql_query("SELECT * FROM contratos");
	while($dato=mysql_fetch_array($can)){
		...
		$objSheet->setCellValue('AO'.$numero, '');
		...
		//if($dato['funciona'] == 'si') {
		//	if($dato['formula'] == 'luz') {
		//		$objSheet->setCellValue('AU'.$numero, 'SOLO LUZ');
		//	} elseif($dato['formula'] == 'gas') {
		//		$objSheet->setCellValue('AU'.$numero, 'DUAL');
		//	} 
		//	$objSheet->setCellValue('AV'.$numero, $dato['modal_funciona']);
		//}
		...
		//if($dato['direccion_envio_facturas'] == ''){
		//	$objSheet->setCellValue('BF'.$numero, 'PS');
		//} elseif($dato['calle_ps'] == $dato['calle_ef_otra']) {
		//	$objSheet->setCellValue('BF'.$numero, 'CLIENTE');
		//} else {
		...
		if($dato['relacion_representante_titular'] == 'Cónyuge/pareja registrada'){
			...
			$objSheet->setCellValue('BK'.$numero, '');
		}
		...
		if($dato['cliente_municipio_otra'] == '') {
			$objSheet->setCellValue('BN'.$numero, '');
		} else {
		...
		if($dato['municipio_ef_otra'] == ''){
			$objSheet->setCellValue('BZ'.$numero, '');
		} else {
...
echo 'Archivo guardado en ' . $carpeta . "\CARGA MASIVA";
echo '<script>
    swal({
      type: "success",
	...
	';
echo '<meta http-equiv="refresh" content="2; url='. $site .'/index.php?page=home" />';
?>