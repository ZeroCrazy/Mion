...
$objSheet->setCellValue('CQ1', 'EDP_CLICK');
	...
	$numero=1;
if($_GET[cod_comercial] == ''){
	if($_GET[intro_dia] == ''){
		$can=mysql_query("SELECT * FROM contratos WHERE intro_mes='$_GET[intro_mes]' AND intro_ano='$_GET[intro_ano]'");
	} else {
		$can=mysql_query("SELECT * FROM contratos WHERE intro_dia='$_GET[intro_dia]' AND intro_mes='$_GET[intro_mes]' AND intro_ano='$_GET[intro_ano]'");
	}
} else {
	if($_GET[intro_dia] == ''){
		$can=mysql_query("SELECT * FROM contratos WHERE user_contrato_intro='$_GET[cod_comercial]' AND intro_mes='$_GET[intro_mes]' AND intro_ano='$_GET[intro_ano]'");
	} else {
		$can=mysql_query("SELECT * FROM contratos WHERE user_contrato_intro='$_GET[cod_comercial]' AND intro_dia='$_GET[intro_dia]' AND intro_mes='$_GET[intro_mes]' AND intro_ano='$_GET[intro_ano]'");
	}
}
	//$can=mysql_query("SELECT * FROM contratos WHERE codigo_comercial='$_GET[cod_comercial]'");
...
if($_GET[cod_comercial] == ''){
	if($_GET[intro_dia] == ''){
		$objWriter->save($carpeta . "\EXPORT_MES_$_GET[intro_mes]$_GET[intro_ano].xls");
		...
		$objWriter->save($carpeta . "\EXPORT_FECHA_$_GET[intro_dia]$_GET[intro_mes]$_GET[intro_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_FECHA_'. $_GET[intro_dia] .''. $_GET[intro_mes] .''. $_GET[intro_ano] .'.xls" />';
	}
} else {
	if($_GET[intro_dia] == ''){
		$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[cod_comercial]) ."_FECHA_$_GET[intro_mes]$_GET[intro_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. $_GET[cod_comercial] .'_FECHA_'. $_GET[intro_mes] .''. $_GET[intro_ano] .'.xls" />';
	} else {
		$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[cod_comercial]) ."_FECHA_$_GET[intro_dia]$_GET[intro_mes]$_GET[intro_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. $_GET[cod_comercial] .'_FECHA_'. $_GET[intro_dia] .''. $_GET[intro_mes] .''. $_GET[intro_ano] .'.xls" />';
	}
}
