...
if($_GET['type'] == 'contrato'){
	...
	if($_GET['codv']){
			$can=mysql_query("SELECT * FROM contratos WHERE cod_verificacion='$_GET[codv]'");
	}
	if($_GET['id']){
			$can=mysql_query("SELECT * FROM contratos WHERE id='$_GET[id]'");
	}
	//if($_GET['page'] == 'we_dia'){
	//	// http://10.34.1.10/excel/Contratos.php?type=contrato&type2=oficial&type3=weekend&gerente=MAXI&we_dia=18&we_mes=05&we_ano=2018
	//	$can=mysql_query("SELECT * FROM contratos WHERE subgerente2='$_GET[gerente]' AND we_dia='$_GET[we_dia]' AND we_mes='$_GET[we_mes]' AND we_ano='$_GET[we_ano]'");
	//}
	//if($_GET['intro_dia']){
	//	$can=mysql_query("SELECT * FROM contratos WHERE subgerente2='$_GET[gerente]' AND intro_dia='$_GET[intro_dia]' AND intro_mes='$_GET[intro_mes]' AND intro_ano='$_GET[intro_ano]'");
	...
	while($dato=mysql_fetch_array($can)){
		...
		if($dato['representante_nombre'] == '') {
			$objSheet->setCellValue('A'.$numero, 'CARGA INMEDIATA');
		} else {
			$objSheet->setCellValue('A'.$numero, 'TRAS PLAZO DE DESISTIMIENTO');
		}
		if($_GET['type'] == 'void'){
		...
		if($dato['cod_verificacion_posterior'] == 'posterior'){
			$tcodv = "P"; // Type Code Verificaction
		} else {
			$tcodv = "V";
		}
		$objSheet->setCellValue('AK'.$numero, $tcodv.$dato['cod_verificacion']);
		...
		$objSheet->setCellValue('BO'.$numero, 'NO');
...
if($_GET['type'] == 'void'){
		if($_GET['type3'] == 'fechaintro'){
			$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[gerente]) ."_". date("dmY") .".xls");	
			echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. utf16_2_utf8($_GET[gerente]) .'_'. date("dmY") .'.xls" />';
		} else {
			$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[gerente]) ."_WE_". $_GET[we_dia] .".". $_GET[we_mes] .".xls");	
			echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. utf16_2_utf8($_GET[gerente]) .'_WE_'. $_GET[we_dia] .'.'. $_GET[we_mes] .'.xls" />';
		}
}
if($_GET['type'] == 'contrato'){
		if($_GET['type3'] == 'fechaintro'){
...
if($_GET['codv']){
	$objWriter->save($carpeta . "\Contrato_". $_GET[codv] .".xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/Contrato_'. $_GET[codv] .'.xls" />';
}
if($_GET['id']){
	$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[idsubgerente2]) ."_". date("dmY") .".xls");
	
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. $_GET['idsubgerente2'] .'_'. date("dmY") .'.xls" />';
}
