...
$objXLS->getActiveSheet()->getStyle('BN1:CR1')->applyFromArray(array('font' => array('color' => array('rgb' => 'ffffff')))); // Color blanco
...
$objSheet->setCellValue('CR1', 'ESTADO');
	...
	if($_GET['gerente']) {
		...
		$can=mysql_query("SELECT * FROM contratos WHERE we_dia='$_GET[we_dia]' AND we_mes='$_GET[we_mes]' AND we_ano='$_GET[we_ano]'");
	}
	//$can=mysql_query("SELECT * FROM contratos WHERE codigo_comercial='$_GET[gerente]'");
	while($dato=mysql_fetch_array($can)){
		...
		// DIRECCIÓN ENVÍO FACTURAS
		if($dato['calle_ef_otra'] == ''){
			$objSheet->setCellValue('BF'.$numero, 'PS');
		} elseif($dato['calle_ef_otra'] == $dato['cliente_calle_otra']){
			$objSheet->setCellValue('BF'.$numero, 'OTRA');
			$objSheet->setCellValue('BZ'.$numero, substr($dato['cliente_cod_postal_otra'], -8, 2).' - '.$dato['cliente_municipio_otra']);
			$objSheet->setCellValue('CA'.$numero, $dato['cliente_municipio_otra']);
			$objSheet->setCellValue('CB'.$numero, $dato['cliente_poblacion_otra']);
			$objSheet->setCellValue('CC'.$numero, $dato['cliente_tipo_via_otra']);
			$objSheet->setCellValue('CD'.$numero, $dato['cliente_calle_otra']);
			$objSheet->setCellValue('CE'.$numero, $dato['cliente_numero_otra']);
			$objSheet->setCellValue('CF'.$numero, '');
			$objSheet->setCellValue('CG'.$numero, $dato['cliente_escalera_otra']);
			$objSheet->setCellValue('CH'.$numero, $dato['cliente_piso_otra']);
			$objSheet->setCellValue('CI'.$numero, $dato['cliente_letra_otra']);
			$objSheet->setCellValue('CJ'.$numero, $dato['cliente_cod_postal_otra']);
		} else {
			$objSheet->setCellValue('BF'.$numero, 'OTRA');
		...
		$objSheet->setCellValue('CR'.$numero, $dato['estado_contrato']);
	}
...
if($_GET[gerente] == ''){
	if($_GET[we_dia] == ''){
		$objWriter->save($carpeta . "\EXPORT_MES_$_GET[we_mes]$_GET[we_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_MES_'. $_GET[we_mes] .''. $_GET[we_ano] .'.xls" />';
	} else {
		$objWriter->save($carpeta . "\EXPORT_WE_$_GET[we_dia]$_GET[we_mes]$_GET[we_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_WE_'. $_GET[we_dia] .''. $_GET[we_mes] .''. $_GET[we_ano] .'.xls" />';
	}
} else {
	if($_GET[we_dia] == ''){
		$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[gerente]) ."_WE_$_GET[we_mes]$_GET[we_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. $_GET[gerente] .'_WE_'. $_GET[we_mes] .''. $_GET[we_ano] .'.xls" />';
	} else {
		$objWriter->save($carpeta . "\EXPORT_". utf8_decode($_GET[gerente]) ."_WE_$_GET[we_dia]$_GET[we_mes]$_GET[we_ano].xls");
		echo '<meta http-equiv="refresh" content="1; url='. $site .'/excel/CARGA EDP_'. date(Y) .''. date(m) .''. date(d) .'/EXPORT_'. $_GET[gerente] .'_WE_'. $_GET[we_dia] .''. $_GET[we_mes] .''. $_GET[we_ano] .'.xls" />';
	}
}
