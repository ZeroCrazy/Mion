...
<?php
$Var = mysql_query("SELECT * FROM cigs_copy ORDER BY id DESC LIMIT 1");
...
<?php
$hora_reset = 131240; // Se vaciará la tabla 'cigs' a partir de las 23:00:00 (230000)
$hora = new DateTime("". date(d) ."-". date(m) ."-". date(y) ." ". date(His) ."");
$hora_prox = new DateTime("". date(d) ."-". date(m) ."-". date(y) ." ". $hora_reset ."");
if($hora == $hora_prox){
	mysql_query("UPDATE cigs_copy SET num='-1', fecha='". date(d) . '-' . date(m) . '-' . date(Y) . ' ' . date(H) . ':' . date(i) . ':' . date(s) ."', user_intro='SERVIDOR' WHERE id='1'");
